...
document.addEventListener("DOMContentLoaded", () => {
        ...
        <div class="prod-card-new" onclick="window.location.href='${rootPath}public/pages/product-details.html?id=${p.id}'">
          ...
          <div class="prod-body">
            ...
            <div class="prod-foot-new">
              ...
              <button class="prod-add-btn mono open-rx-modal-btn" style="background:var(--accent); color:#fff; border-color:var(--accent);" onclick="event.stopPropagation(); window.openRxModal();">${p.prescriptionRequired ? 'UPLOAD RX' : 'ORDER'}</button>
            </div>
          </div>
        </div>
      `).join("");
  ...
  if (pathname.includes("product-details.html")) {
    ...
    // Connect Prescription Order button
    const cartBtn = document.getElementById("productDetailCartBtn");
    if (cartBtn) {
      cartBtn.innerText = product.prescriptionRequired ? "Upload Prescription for Home Delivery →" : "Order from Local Store →";
